...
\begin{minipage}[t]{0.48\linewidth}
  \vspace{0pt}
  \subsubsection{Mathematics of Matrix Conversion}

  If \(q_k\) is the \(k\)-th stored correction and the scan contains \(N_x\)
  points per line, the planned converter maps the one-dimensional sequence
  into rows and columns using
  \[
    Q_{j,i}=q_{jN_x+i},
    \qquad
    0\leq i<N_x,\quad 0\leq j<N_y .
  \]
  A measured vertical calibration factor \(\alpha\) would later convert
  relative controller counts into height differences:
  \[
    \Delta z_{j,i}
      =\alpha\left(Q_{j,i}-Q_{\mathrm{ref}}\right).
  \]
  \noindent\fbox{\parbox[c][0.40in][c]{0.92\linewidth}{
    \centering
    \textbf{CALIBRATION PLACEHOLDER:}
    \(\alpha=\rule{0.28\linewidth}{0.4pt}\)
    distance/count; source and method:
...
\begin{minipage}[t]{0.48\linewidth}
  \vspace{0pt}
  \subsubsection{Python Conversion Code}

  \begin{center}
    \fbox{\parbox[c][1.62in][c]{0.91\linewidth}{
      \centering
      \textbf{PYTHON CODE-SNIPPET PLACEHOLDER}\\[5pt]
      Insert the tested file reader, length check, matrix reshape, backward
      alignment, and raw-matrix save steps here.\\[5pt]
      \textit{The conversion program has not yet been written.}
    }}
    \captionsetup{font=footnotesize}
    \captionof{figure}{Placeholder for a readable excerpt from the completed
    and repository-linked Python converter.}
  \end{center}
\end{minipage}
...
\begin{center}
  \fbox{\parbox[c][2.55in][c]{0.90\linewidth}{
    \centering
    \textbf{SURFACE-RECONSTRUCTION IMAGE PLACEHOLDER}\\[6pt]
