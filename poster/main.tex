...
%
% We still need to completely write the operating principles page.
% That means tunneling current, coarse and fine positioning,
% constant-current imaging, and the actual measurement procedure.
% - Justin
%
...
The tunneling signal is the physical basis of STM operation. A voltage bias
between a conductive sample and a sharp metallic tip allows electrons to
quantum-mechanically tunnel across the small gap. The resulting current varies
approximately exponentially with tip--sample separation, so even a very small
change in distance produces a measurable change in signal. The sample must be
conductive to complete the tunneling circuit, and the tip must be extremely
sharp so that the measurement is localized to a very small region of the
surface.

\subsection{Coarse and Fine Positioning}

% Add how the coarse motor, z piezo, and x-y movement each work.
The coarse motor, located above the piezoelectric scanner, moves the tip toward
the sample until the separation is within the fine-positioning range. The
\(z\)-piezo then adjusts the vertical tip position in response to the measured
tunneling current, while the \(x\)- and \(y\)-piezo signals move the tip
laterally across the sample. Piezoelectric materials expand or contract by
sub-nanometer amounts when voltage is applied, allowing the feedback system to
make the small corrections needed during a scan.

% Justin: add more detail on the coarse-motor mechanism.
...
During constant-current imaging, the feedback loop adjusts the \(z\)-piezo to
hold the tunneling current near its setpoint as the tip moves across the
surface. At each lateral position, the required \(z\) correction records the
relative vertical response needed to preserve the current. Before a
vertical-piezo calibration is measured, these values are reported as relative
correction or DAC counts rather than absolute surface height.
...
\subsection{Firmware Scan-Pattern Sequence}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\textwidth]{scan-pattern-sequence.png}
  \caption{Selected frames from the Python scan-pattern animation. The
  sequence follows the firmware's Timer~2 routine: sample while stepping in
  \(+x\), sample while returning in \(-x\), reset \(x\), increment \(y\), and
  begin the next line. The displayed \texttt{pointcounter} values follow the
  source literally.\textsuperscript{[2,4]}}
\end{figure}

\noindent
The complete animation and its reproducible Python source are available as
\href{https://github.com/souuk/clustered/blob/main/animate_scan_pattern.py}
{\nolinkurl{animate_scan_pattern.py}} and
\href{https://github.com/souuk/clustered/blob/main/scan-pattern-preview.gif}
{\nolinkurl{scan-pattern-preview.gif}}. The model is based on scan
initialization at lines 838--857 and raster execution at lines
1178--1229 of the Teensy source.\textsuperscript{[2,4]}
