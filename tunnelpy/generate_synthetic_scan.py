...
These files were generated only because the borrowed scans in data\\ are
incomplete, appended, or otherwise unverified. Do not describe SYNF1.hex,
SYNB1.hex, or SYNP1.txt as measurements from the microscope.
...
Borrowed-data basis (format and controller behavior only):
  data\\STM1F8.hex: 65,431 samples; 255 complete 256-point rows
  data\\STM1B8.hex: 65,428 samples; 255 complete 256-point rows
  observed signed-Q range was approximately -32,302 to +28,826 counts
  strong slow line-to-line drift and close forward/backward correspondence
  borrowed values remain unverified and were not copied into this simulation
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--figure",
        type=Path,
        default=Path("transform/raw-data-to-matrix.png"),
        help="raw-data-to-matrix PNG destination",
    )
    parser.add_argument(
        "--schema-figure",
        type=Path,
        default=Path("transform/file-schema-validation.png"),
        help="compact F/B/P integrity-check PNG destination",
    )
    parser.add_argument(
        "--four-panel",
        type=Path,
        default=Path("transform/synthetic-q-four-panel.png"),
